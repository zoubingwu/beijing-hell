...
4. Win32 整数溢出不视为可移植行为；Web 将经济数值限制在 JavaScript 安全整数域，达到边界时饱和，并用 BigInt 计算需要精确中间值的加权均价与财富。这是已批准的可移植性偏差，可避免精度丢失、Infinity 和存档失效。
...
| 结算 | 已实施（Plan 006） | `SelectionDlg.cpp:1491-1542,1892-1953` | 自动结束按最终报价清仓；手动结束不清仓；普通死亡不评分，末日死亡继续自动结算 |
| 排行榜 | 已实施（Plan 006） | `TopPlayerDlg.cpp:14-26,80-113,142-313` | 预置 Top 10、财富门槛、同分插前、姓名/健康/名声字段；复刻 10–19 名声 bug（`src/game/scoring.test.ts`） |
| 持久化 | 已实施（Plan 007，schema 6） | `beijing-hell:save:v6`；v1-v5 key 保留且不读取 | 活动局与排行榜独立校验/回退；只保存和清除 v6；损坏活动局不丢有效榜单，损坏榜单不丢有效活动局 |
