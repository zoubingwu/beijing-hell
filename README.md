...
游戏会自动保存到浏览器 localStorage。当前使用 schema 6 存档键 `beijing-hell:save:v6`，将活动局与 Top 10 排行榜分离保存；活动局损坏时仅新建活动局，仍保留有效排行榜。旧版 `beijing-hell:save:v1` 至 `beijing-hell:save:v5` 会被刻意保留且不读取、不覆盖，按最终迁移政策不再猜测性迁移。完成一局后，最终资产必须为正且达到当前榜尾门槛（榜单不足十人时只要求为正），才可进入本机 Top 10。
